...
int timer_subscribe_int(unsigned int *hook_id) // Modified for mouse
{
	int ret = 0;
...
int timer_unsubscribe_int(unsigned int *hook_id) // Modified for mouse
{
	int ret = 0;
...
int timer_test_int(unsigned long time) {
	...
	int hook_id = 1;
	int irq_set = BIT(timer_subscribe_int(&hook_id)); // Modified for mouse
	int ipc_status;
	message msg;
	...
	while (counter < time * 60) { /* You may want to use a different condition */
	 ...
	 // Modified for mouse
	if (timer_unsubscribe_int(&hook_id) != OK) // If an error has occurred before, it will show up now and return 1
		ret = 1;
